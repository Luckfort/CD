...
class DataProcessing():
    ...
    def dispacher(self):
        ...
        if self.data_name == 'StrategyQA':
            p, q = self.StrategyQA()
            prompt = 'Judge whether the answer of the question is true.  Please answer the question in the following format: first say "Yes" or "No", then tell the reasons.'  + '\n' \
            ...
            p, q = self.coinflip()
            prompt = 'According to the flipping process above, determine if a coin remains heads up after it is either flipped or left unflipped by individuals. Please answer the question in the following format: first say "Yes" or "No", then tell the reasons. '
            
        elif self.data_name == 'cities':
            p, q = self.cities()
            prompt = 'Judge whether the statement is True or not. Your answer should be in the following format. Answer: {Yes or No}. Reason: {Why you choose that answer}'
            ...
            p, q = self.hateeval()
            prompt = 'According to the comment, tell whether they present hate speech or not. Please answer the question in the following format: first say "Yes" or "No", then tell the reasons.'
            
        elif self.data_name == 'STSA':
            p, q = self.STSA()
            prompt = 'The sentence above is a movie review and reflects the writer\'s overall intention for this review. According to the sentence, judge whether the emotion is Positive or Negative. If Positive, please say yes ahead of the answer. If Negative, please say no ahead of the answer. Please answer the question in the following format: first say "Yes" or "No", then tell the reasons.'
            
        elif self.data_name == 'IMDb':
            p, q = self.IMDb()
            prompt = 'According to the movie review, judge whether it is Positive or Negative. If Positive, please say yes ahead of the answer. If Negative, please say no ahead of the answer. Please answer the question in the following format: first say "Yes" or "No", then tell the reasons.'
            ...
            cot = 'Think carefully according to the sentence. Is there any sarcasm in this sentence? Please answer the question in the following format: first say "Yes" or "No", then tell the reasons.'   
